...
// Define ABI and addresses for Aave
const AAVE_LENDING_POOL_ADDRESS_PROVIDER_ABI = /* ABI */;
const AAVE_LENDING_POOL_ADDRESS_PROVIDER_ADDRESS = process.env.AAVE_LENDING_POOL_ADDRESS_PROVIDER_ADDRESS;
const AAVE_LENDING_POOL_ABI = /* ABI */;
...
// Fetch real-time gas price from the API
async function fetchRealTimeGasPrice() {
    const response = await axios.get('https://cointool.app/gasPrice/eth');
    return parseFloat(response.data.data.fast);
}
...
async function fetchPriceFromGraphAPI(dex, token0Address, token1Address) {
    const query = `{
        pair(id: "${token0Address.toLowerCase()}-${token1Address.toLowerCase()}") {
            token0 { id }
            token1 { id }
            reserve0
            reserve1
